...
with sync_playwright() as p:
    ...
    print(
        f"   Scale:      {profile['device_scale_factor']}x {'(Retina)' if profile['device_scale_factor'] == 2 else ''}")
    ...
    if captured_request.get("response", {}).get("status") == "success":
        picture_clean = picture.replace('«', '"').replace('»', '"')
        result = f"Заявка отправлена!\nИмя: {name}\nТелефон: {phone}\nEmail: {email}\nКартина: {picture_clean}"
    else:
        result = "❌ ОШИБКА — форма не отправила заявку"
